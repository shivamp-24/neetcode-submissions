...
public:
    vector<vector<string>> groupAnagrams(vector<string>& strs) {
        ...
        //this is also not good -> m * logm * n

        //use the same approach as creating a vector of size 26
        // but convert its frequencies into a string
        // then we can use unordered map
        unordered_map<string, vector<string>> mp;
        for(auto str: strs) {
            vector<int> hsh(26, 0);
            ...
            string currHash = "";
            for(int x: hsh) {
                // currHash += to_string(x);
                // this is not good, we need to add separation between frequencies
                // example -> bdddddddddd","bbbbbbbbbbc
                // frequencies will be 0 1 0 10, and 0 10 1 0
                // which when converted to string yields the same result

                currHash += to_string(x) + ',';
            }
            mp[currHash].push_back(str);
        }
        vector<vector<string>> res;
        for(auto it: mp) res.push_back(it.second);
